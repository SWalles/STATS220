...
Each photo in pixabay has a user associated with it and our selection of photos are associated with `r num_users` different users.
...
From my investigation I found that the percentage of people who downloaded a photo after viewing it was between `r round(mid_75_down_view_percentage[1], 1)`% and `r round(mid_75_down_view_percentage[2], 1)`% for 75% of the photos.
...
These results were surprising as the `r with(likes_by_pop, popularity[mean_likes == max(mean_likes)])` category got the highest mean like score and the `r with(likes_by_pop, popularity[mean_likes == min(mean_likes)])` category got the lowest mean like score.
